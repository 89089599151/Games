...
def can_control_turn(game: ChatGame, user_id: int) -> bool:
    ...
    if not turn:
        return False
    # На одном телефоне хост управляет всеми действиями, но оставляем
...
async def cb_add_gender(c: CallbackQuery):
    ...
    confirmation = (
        f"✅ Добавлен игрок <b>{html.escape(player.name)}</b> "
        f"({info['emoji']} {info['title']})."
    )
    await c.message.answer(confirmation)
...
async def cb_pick_type(c: CallbackQuery):
    ...
    # хост управляет ходом с одного телефона или сам игрок
    turn = game.current_turn
